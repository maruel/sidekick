...
if [ -S "/tmp/.X11-unix/X${DISPLAY}" ] 2>/dev/null; then
    echo "Xvfb already running on DISPLAY=:${DISPLAY}, will reuse"
...
if [ "$EMULATOR_ALREADY_RUNNING" = false ]; then
    if [ "$XVFB_ALREADY_RUNNING" = false ]; then
        ...
        echo "Reusing existing Xvfb instance"
        XVFB_PID=""
    fi
    ...
    echo "Skipping emulator boot, already running"
fi

echo "Clearing app state before tests..."
adb uninstall com.fghbuild.sidekick >/dev/null 2>&1 || true
adb shell pm clear com.fghbuild.sidekick >/dev/null 2>&1 || true
adb shell rm -rf /data/user/0/com.fghbuild.sidekick >/dev/null 2>&1 || true
adb shell rm -rf /data/data/com.fghbuild.sidekick >/dev/null 2>&1 || true
adb shell rm -rf /data/app/*/app_dxmaker_cache >/dev/null 2>&1 || true

cleanup() {
    echo "Cleaning up app state..."
    adb uninstall com.fghbuild.sidekick >/dev/null 2>&1 || true
    adb shell pm clear com.fghbuild.sidekick >/dev/null 2>&1 || true
    adb shell rm -rf /data/user/0/com.fghbuild.sidekick >/dev/null 2>&1 || true
    adb shell rm -rf /data/data/com.fghbuild.sidekick >/dev/null 2>&1 || true
